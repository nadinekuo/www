...
if(this._options['validate']){cites=array();for(var i=0;i<this.data.length;i++){cites[cites.length]=this.data[i]['cite'];}
unique=array_unique(cites);if(sizeof(cites)!=sizeof(unique)){notuniques=array();for(var i=0;i<sizeof(cites);i++){if(''==unique[i]){notuniques[notuniques.length]=cites[i];}}
...
var arr=split('{',entry);ret['cite']=trim(arr[1]);ret['entryType']=strtolower(trim(arr[0]));if('@'==ret['entryType'].substring(0,1)){ret['entryType']=substr(ret['entryType'],1);}
...
if(3==sizeof(tmparray)){jr=tmparray[1];}
...
{if(typeof wholeentry=='undefined')wholeentry='';warning['warning']=type;warning['entry']=entry;warning['wholeentry']=wholeentry;this.warnings[this.warnings.length]=warning;},'clearWarnings':function()
...
if(array_key_exists('author',entry)){if(this._options['extractAuthors']){tmparray=array();for(j in entry['author']){var authorentry=entry['author'][j];tmparray[tmparray.length]=this._formatAuthor(authorentry);}
...
if(array_key_exists('author',entry)){if(this._options['extractAuthors']){tmparray=array();for(var j in entry['author']){var authorentry=entry['author'][j];tmparray[tmparray.length]=this._formatAuthor(authorentry);}
